...
async function renderStylesTab(container) {
    ...
    const editingId = { value: null };
    const formTitle = document.createElement("h3");
    formTitle.id = "fria-style-form-title";
    formTitle.textContent = "Nouveau style";
    Object.assign(formTitle.style, { margin: "0 0 10px", fontSize: "11px", color: "#888", fontWeight: "600" });
    form.appendChild(formTitle);

    const mkField = (label, value = "", isTextarea = false, placeholder = "") => {
        const wrap = document.createElement("div");
        wrap.style.cssText = "margin-bottom:8px;";
        const l = document.createElement("label");
        l.textContent = label;
        ...
        if (isTextarea) {
            input.rows = 3;
            input.style.cssText = _friaStyle.input + "resize:vertical;min-height:60px;";
        } else {
            input.type = "text";
            input.style.cssText = _friaStyle.input;
        }
    ...
    const fName = mkField("Nom du style", "", false, "");
    form.appendChild(fName.wrap);
    const fStyleText = mkField("Texte du style", "", true, "Description du style, ex: 'cyberpunk, neon lights, rain'");
    form.appendChild(fStyleText.wrap);
    const fNegPrompt = mkField("Negative prompt (optionnel)", "", true, "ex: 'lowres, bad anatomy, blurry'");
    form.appendChild(fNegPrompt.wrap);
    ...
    const fPublic = mkCheck("Public (visible par tous)", false);
    checks.appendChild(fPublic.wrap);
    form.appendChild(checks);
    ...
    const cancelBtn = mkBtn("Annuler", _friaStyle.btnSecondary, () => {
        editingId.value = null;
        fName.input.value = ""; fStyleText.input.value = ""; fNegPrompt.input.value = "";
        fPublic.input.checked = false;
        formTitle.textContent = "Nouveau style";
    });
    const saveBtn = mkBtn("Sauvegarder", _friaStyle.btn(), async () => {
        const body = {
            name: fName.input.value.trim(),
            style_text: fStyleText.input.value.trim(),
            negative_prompt: fNegPrompt.input.value.trim(),
            is_public: fPublic.input.checked ? 1 : 0,
        };
        if (!body.name || !body.style_text) {
            alert("Nom et texte du style sont requis"); return;
        }
        try {
            if (editingId.value) {
                await _friaFetchApi(`styles/${editingId.value}`, { method: "PUT", body });
            } else {
                await _friaFetchApi("styles", { method: "POST", body });
            }
            editingId.value = null;
            formTitle.textContent = "Nouveau style";
            fName.input.value = ""; fStyleText.input.value = ""; fNegPrompt.input.value = "";
    ...
    async function reloadStyles() {
        ...
        listSection.innerHTML = "";
        const t = document.createElement("h3");
        t.textContent = "MES STYLES";
        Object.assign(t.style, { margin: "0 0 8px", fontSize: "11px", color: "#888", fontWeight: "600" });
        listSection.appendChild(t);
        if (styles.length === 0) {
            const empty = document.createElement("p");
            empty.textContent = "Aucun style. Créez-en un ci-dessous.";
            Object.assign(empty.style, { color: "#888", fontSize: "12px", margin: "0" });
            listSection.appendChild(empty);
            return;
        }
        styles.forEach(s => {
            ...
            const left = document.createElement("div");
            const scope = s.is_public ? "public" : (s.owner_name ? `(${s.owner_name})` : "personnel");
            const preview = (s.style_text || "").substring(0, 60);
            left.innerHTML = `<strong style='color:#fff;'>${s.name}</strong> <span style='color:#888;'>[${scope}]</span><br><span style='color:#aaa;font-size:11px;'>${preview}${preview.length >= 60 ? "..." : ""}</span>`;
            const actions = document.createElement("div");
            Object.assign(actions.style, { display: "flex", gap: "4px" });
            const editBtn = mkBtn("Edit", _friaStyle.btnSecondary, () => fillForm(s));
            const dupBtn = mkBtn("Dup", _friaStyle.btnSecondary, async () => {
                const body = { name: s.name + " (copie)", style_text: s.style_text, negative_prompt: s.negative_prompt || "", is_public: 0 };
                try {
                    await _friaFetchApi("styles", { method: "POST", body });
                    reloadStyles();
                } catch (e) { alert("Erreur duplication : " + e.message); }
            ...
            const delBtn = mkBtn("Del", "padding:6px 12px;border-radius:4px;border:none;background:#7f1d1d;color:white;cursor:pointer;font-size:12px;", async () => {
                ...
                try {
                    await _friaFetchApi(`styles/${s.id}`, { method: "DELETE" });
                    reloadStyles();
                } catch (e) { alert("Erreur suppression : " + e.message); }
            });
            actions.append(editBtn, dupBtn, delBtn);
    ...
    function fillForm(s) {
        ...
        fName.input.value = s.name || "";
        fStyleText.input.value = s.style_text || "";
        fNegPrompt.input.value = s.negative_prompt || "";
        fPublic.input.checked = !!s.is_public;
        formTitle.textContent = "Modifier style";
    }
}

// ── Onglet Templates (CRUD complet) ───────────────────────────────
...
async function renderTemplatesTab(container) {
    ...
    const editingId = { value: null };
    const formTitle = document.createElement("h3");
    formTitle.id = "fria-tmpl-form-title";
    formTitle.textContent = "Nouveau template";
    Object.assign(formTitle.style, { margin: "0 0 10px", fontSize: "11px", color: "#888", fontWeight: "600" });
    form.appendChild(formTitle);

    // Sélecteurs type / format
    ...
    const mkSelect = (label, options, value) => {
        ...
        sel.style.cssText = _friaStyle.input;
        options.forEach(o => {
    ...
    const fType = mkSelect("Prompt type", ["sdxl", "sd15", "flux", "anima", "qwen", "ideogram4", "liste"], "sdxl");
    const fFormat = mkSelect("Output format", ["text", "markdown", "json"], "text");
    selRow.append(fType.wrap, fFormat.wrap);
    form.appendChild(selRow);

    const mkTextarea = (label, value = "", rows = 6, placeholder = "") => {
        const wrap = document.createElement("div");
        wrap.style.cssText = "margin-bottom:8px;";
        const l = document.createElement("label");
        l.textContent = label;
        ...
        ta.style.cssText = _friaStyle.input + "resize:vertical;min-height:80px;font-family:monospace;";
        wrap.appendChild(ta);
        return { wrap, input: ta };
    };

    const fSystem = mkTextarea("System prompt", "", 8, "Le system prompt complet envoyé au LLM...");
    form.appendChild(fSystem.wrap);

    // Exemples (un par ligne)
    const fExamples = mkTextarea("Exemples (un par ligne)", "", 4, "Exemple de prompt 1\nexemple de prompt 2");
    form.appendChild(fExamples.wrap);

    const btnRow = document.createElement("div");
    btnRow.style.cssText = "display:flex;gap:8px;justify-content:flex-end;";
    const cancelBtn = mkBtn("Annuler", _friaStyle.btnSecondary, () => {
        editingId.value = null;
        fSystem.input.value = ""; fExamples.input.value = "";
        formTitle.textContent = "Nouveau template";
    });
    const saveBtn = mkBtn("Sauvegarder", _friaStyle.btn(), async () => {
        const body = {
            prompt_type: fType.sel.value,
            output_format: fFormat.sel.value,
            system_prompt: fSystem.input.value.trim(),
            examples: fExamples.input.value.split("\n").map(s => s.trim()).filter(Boolean),
        };
        if (!body.system_prompt) {
            alert("Le system prompt est requis"); return;
        }
        try {
            if (editingId.value) {
                await _friaFetchApi(`prompts/templates/${editingId.value}`, { method: "PUT", body });
            } else {
                await _friaFetchApi("prompts/templates", { method: "POST", body });
            }
            editingId.value = null;
            formTitle.textContent = "Nouveau template";
            fSystem.input.value = ""; fExamples.input.value = "";
    ...
    async function reloadTemplates() {
        // Charge les templates utilisateur (et les défauts en lecture seule)
        const templates = await _friaFetchApi("prompts/templates");
        listSection.innerHTML = "";
        const t = document.createElement("h3");
        t.textContent = "MES TEMPLATES";
        Object.assign(t.style, { margin: "0 0 8px", fontSize: "11px", color: "#888", fontWeight: "600" });
        listSection.appendChild(t);
        if (templates.length === 0) {
            const empty = document.createElement("p");
            empty.textContent = "Aucun template personnalisé. Les templates par défaut ne sont pas listés ici (non éditables).";
            Object.assign(empty.style, { color: "#888", fontSize: "12px", margin: "0" });
            listSection.appendChild(empty);
            return;
        }
        templates.forEach(tmpl => {
            ...
            const left = document.createElement("div");
            const editable = tmpl.editable;
            const sysPreview = (tmpl.system_prompt || "").substring(0, 80);
            const lockIcon = editable ? "" : " 🔒";
            left.innerHTML = `<strong style='color:#fff;'>${tmpl.prompt_type} / ${tmpl.output_format}</strong>${lockIcon}<br><span style='color:#aaa;font-size:11px;'>${sysPreview}${sysPreview.length >= 80 ? "..." : ""}</span>`;
            const actions = document.createElement("div");
            Object.assign(actions.style, { display: "flex", gap: "4px" });
            if (editable) {
                const editBtn = mkBtn("Edit", _friaStyle.btnSecondary, () => fillForm(tmpl));
                const delBtn = mkBtn("Del", "padding:6px 12px;border-radius:4px;border:none;background:#7f1d1d;color:white;cursor:pointer;font-size:12px;", async () => {
                    if (!confirm("Supprimer ce template ?")) return;
                    try {
                        await _friaFetchApi(`prompts/templates/${tmpl.id}`, { method: "DELETE" });
                        reloadTemplates();
                    } catch (e) { alert("Erreur suppression : " + e.message); }
                });
                ...
                lockLbl.textContent = "(défaut)";
                Object.assign(lockLbl.style, { color: "#666", fontSize: "10px", fontStyle: "italic" });
    ...
    function fillForm(tmpl) {
        editingId.value = tmpl.id;
        fType.sel.value = tmpl.prompt_type || "sdxl";
        fFormat.sel.value = tmpl.output_format || "text";
        fSystem.input.value = tmpl.system_prompt || "";
        fExamples.input.value = Array.isArray(tmpl.examples) ? tmpl.examples.join("\n") : "";
        formTitle.textContent = "Modifier template";
    }
}
